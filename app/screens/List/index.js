...
export default function List({navigation, route}) {
  ...
  (state)=>console.log('setting sdfsdfa', state);
  ...
  const onChangeMapView = () => {
    ...
    if (!mapView) {
      setRegion({
        latitude: 13.566045491061695,
        longitude: 105.66990591205348,
        latitudeDelta: 13,
        longitudeDelta: 13,
      });
    }
  ...
  const renderMapView = () => {
    ...
    return (
      <View style={{flex: 1}}>
        <MapView provider={PROVIDER_GOOGLE} style={styles.map} region={region} mapType='hybrid' rotateEnabled={false}>
              ...
              >
                <View
                  style={[
                    ...
                    <Card
                    image={item.category.image?.full}
                    >
                    </Card>
                  {/* <Icon
                    name="star"
